...
PIN builds tamper-evident Python tool releases from clean Git commits and exposes the active release through a stable `current` directory. `pin run` and `pin verify` reject protected release content that no longer matches its build-time integrity manifest; declared injected paths and runtime caches remain mutable.
