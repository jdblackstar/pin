...
type integrityBenchShape struct {
	name     string
	files    int
	fileSize int
}

var integrityBenchShapes = []integrityBenchShape{
...
// BenchmarkReleaseIntegrity measures warm-cache full verification of synthetic releases.
...
func BenchmarkReleaseIntegrity(b *testing.B) {
	for _, shape := range integrityBenchShapes {
		b.Run(shape.name, func(b *testing.B) {
			benchmarkReleaseIntegrity(b, shape, fullIntegrityCheck)
		})
	}
}

// BenchmarkReleaseIntegrityCached measures the pin run path: a cached check of
// an unchanged release whose digest cache was seeded by an earlier full check.
// Setup waits out integrityRacyWindow so the files are settled, as they are
// for any release older than a few seconds.
func BenchmarkReleaseIntegrityCached(b *testing.B) {
	for _, shape := range integrityBenchShapes {
		b.Run(shape.name, func(b *testing.B) {
			benchmarkReleaseIntegrity(b, shape, cachedIntegrityCheck)
		})
	}
}

func benchmarkReleaseIntegrity(b *testing.B, shape integrityBenchShape, check integrityCheck) {
	b.StopTimer()
	release := b.TempDir()
	...
	if check == cachedIntegrityCheck {
		time.Sleep(integrityRacyWindow + 100*time.Millisecond)
	}
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		if err := verifyReleaseIntegrity(release, cfg, check); err != nil {
			b.Fatal(err)
		}
	}
}
